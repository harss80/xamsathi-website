        ...
        <section className="py-20 bg-slate-50 border-y border-slate-200">
            <div className="container mx-auto px-4 md:px-6">
                <div className="text-center max-w-3xl mx-auto mb-16">
                    ...
                    <h2 className="text-3xl md:text-5xl font-black text-slate-900 mt-2">More Exams. More Value.</h2>
                    ...
                    {plans.map((p) => (
                        <div
                            ...
                            className={`relative group p-8 rounded-3xl transition-all duration-300 ${p.highlight
                                    ? "bg-slate-900 text-white shadow-xl scale-100 lg:scale-105 z-10 border border-slate-800"
                                    : "bg-white text-slate-900 border border-slate-200 hover:border-blue-300 hover:shadow-lg"
                                }`}
                        >
                            {p.highlight && (
                            ...
                            <div className="text-center border-b border-opacity-20 pb-8 mb-8 border-slate-500">
                                ...
                                <h3 className={`mt-4 text-xl font-bold ${p.highlight ? "text-white" : "text-slate-900"}`}>{p.name}</h3>
                                <p className={`mt-2 text-sm font-medium ${p.highlight ? "text-slate-400" : "text-slate-600"}`}>{p.subtitle}</p>
                            </div>
                            ...
                            <div className="space-y-4 mb-8">
                                {p.includes.map((t) => (
                                    <div key={t} className="flex items-center gap-3">
                                        ...
                                        <span className={`text-sm font-bold tracking-wide ${p.highlight ? "text-slate-200" : "text-slate-700"}`}>{t}</span>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="mt-20 max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="p-10 rounded-3xl bg-white border border-slate-200 shadow-sm">
                        <div className="text-xl font-black text-slate-900">Why Choose XamSathi</div>
                        ...
                        <div className="mt-8 space-y-4">
                            {comparison.map((row) => (
                                <div key={row.feature} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100 last:border-0 last:pb-0">
                                    <div className="text-sm font-bold text-slate-700">{row.feature}</div>
                                    <div className="flex items-center gap-6">
                                        <div className="text-sm font-black text-blue-600 inline-flex items-center gap-1 bg-blue-50 px-3 py-1 rounded-lg">
                    ...
                    <div className="p-10 rounded-3xl bg-blue-600 text-white border border-blue-700 shadow-xl flex flex-col justify-between relative overflow-hidden">
                        ...
                        <div className="mt-8 relative z-10">
                            <Link
                                ...
                                className="inline-flex flex-wrap items-center justify-center gap-2 px-8 py-4 rounded-xl bg-white text-blue-700 font-black shadow-lg hover:bg-slate-50 transition-colors active:scale-95"
                            >
