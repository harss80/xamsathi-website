                ...
                <div className="max-w-5xl mx-auto px-6 py-12 animate-in fade-in duration-500">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-12">
                        <div className="bg-gradient-to-br from-slate-900 to-slate-900 border border-slate-800 rounded-3xl p-8 relative overflow-hidden group">
                            <div className="absolute top-0 right-0 p-8 opacity-10 bg-emerald-500 blur-3xl rounded-full w-48 h-48 group-hover:opacity-20 transition-opacity" />
                            ...
                            <div className="flex gap-2 mb-6">
                                <span className={`px-3 py-1 rounded-full text-sm font-bold border ${score >= 80 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : score >= 50 ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                                    {score >= 80 ? 'Excellent' : score >= 50 ? 'Good Effort' : 'Needs Improvement'}
                                </span>
                            </div>
                            <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
                                <div
                                    className={`h-full rounded-full ${score >= 50 ? 'bg-emerald-500' : 'bg-red-500'} transition-all duration-1000`}
                            ...
                            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex flex-col justify-center items-center">
                                ...
                                <div className="text-3xl font-bold text-white">
                                    {Object.keys(answers).filter(qId => {
                                        let isCorrect = false;
                                        CASE_STUDIES.forEach(cs => cs.questions.forEach(q => { if (q.id === Number(qId) && answers[Number(qId)] === q.correctAnswer) isCorrect = true; }));
                                        return isCorrect;
                                    }).length}
                            ...
                            <div key={caseStudy.id} className="bg-slate-900/50 border border-slate-800 rounded-3xl overflow-hidden">
                                <div className="p-6 md:p-8 bg-slate-900 border-b border-slate-800">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <h3 className="text-xl font-bold text-white">Case Study {caseStudy.id}: {caseStudy.title}</h3>
                                    </div>
                                                ...
                                                <div className="grid md:grid-cols-2 gap-3 mb-6">
                                                    {q.options.map((option, optIdx) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs shrink-0 ${isAnswer ? 'bg-emerald-500 text-white' :
                                                                ...
                                                                {isAnswer && <CheckCircle2 className="w-5 h-5 text-emerald-500 ml-auto" />}
